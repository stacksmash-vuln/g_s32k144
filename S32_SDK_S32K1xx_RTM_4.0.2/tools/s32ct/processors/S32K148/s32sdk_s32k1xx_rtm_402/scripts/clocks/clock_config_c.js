...
for (elementId in HwAbstr.PccClockDesciption){
    if (HwAbstr.clockElementExist(cfg, elementId)) {
        if (HwAbstr.isClockElementUsed(cfg, elementId)) {
            ...
            if (regPCC.indexOf("PORT") == -1 && regPCC.indexOf("EWM") == -1 && regPCC.indexOf("CMP") == -1 && regPCC.indexOf("CRC") == -1 && regPCC.indexOf("DMA") == -1 && regPCC.indexOf("FTFC") == -1 && regPCC.indexOf("FlexCAN") == -1 && regPCC.indexOf("PDB") == -1 && regPCC.indexOf("RTC") == -1 && regPCC.indexOf("SAI") == -1)
            {
                switch (cfg.getBitFieldValueAsBigInteger(regPCC,"PCS"))
                {
                    case 1:
                        numClkSrc = (regPCC.indexOf("FTM") == -1 && regPCC.indexOf("ENET")==-1) ? "CLK_SRC_SOSC_DIV2" : "CLK_SRC_SOSC_DIV1";
                        break;
                    case 2:
                        numClkSrc = (regPCC.indexOf("FTM") == -1 && regPCC.indexOf("ENET")==-1) ? "CLK_SRC_SIRC_DIV2" : "CLK_SRC_SIRC_DIV1";
                        break;
                    case 3:
                        numClkSrc = (regPCC.indexOf("FTM") == -1 && regPCC.indexOf("ENET")==-1) ? "CLK_SRC_FIRC_DIV2" : "CLK_SRC_FIRC_DIV1";
                        break;
                    case 6:
                        numClkSrc = (regPCC.indexOf("FTM") == -1 && regPCC.indexOf("ENET")==-1) ? "CLK_SRC_SPLL_DIV2" : "CLK_SRC_SPLL_DIV1";
                        break;
                    case 7:
                        numClkSrc = (regPCC.indexOf("FTM") == -1 && regPCC.indexOf("ENET")==-1) ? "CLK_SRC_LPO" : "CLK_SRC_OFF";
                        break;
                    default:
...
"clock_manager_user_config_t clockMan1_InitConfig"+cfgIndex+" = {"]);
    ...
    if (cfg.getValueAsText("SimInitialize") == "yes")
    {
        if (cfg.getBitFieldValueAsBigInteger("SIM::MISCTRL0","QSPI_CLK_SEL") != null)
        {
            CWriter.line("        .qspiRefClkGating =");
            CWriter.line("        {");
            if (cfg.getBitFieldValueAsBigInteger("SIM::MISCTRL0","QSPI_CLK_SEL") == 1)
            {
                CWriter.line("            .enableQspiRefClk = true, /*!< QSPI internal reference clock gate       */");
            }
            else
            {
                CWriter.line("            .enableQspiRefClk = false, /*!< QSPI internal reference clock gate       */");
            }
            CWriter.line("        },");
        }
    }
    ...
    if (cfg.getValueAsText("TCLKConfig") == "yes")
    {
        ...
        CWriter.line("            .extPinSrc[4] = " + cfg.getBitFieldValueAsBigInteger("SIM::FTMOPT0","FTM4CLKSEL") + "U, /*!< FTM4 ext pin source   */");
        CWriter.line("            .extPinSrc[5] = " + cfg.getBitFieldValueAsBigInteger("SIM::FTMOPT0","FTM5CLKSEL") + "U, /*!< FTM5 ext pin source   */");
        CWriter.line("            .extPinSrc[6] = " + cfg.getBitFieldValueAsBigInteger("SIM::FTMOPT0","FTM6CLKSEL") + "U, /*!< FTM6 ext pin source   */");
        CWriter.line("            .extPinSrc[7] = " + cfg.getBitFieldValueAsBigInteger("SIM::FTMOPT0","FTM7CLKSEL") + "U, /*!< FTM7 ext pin source   */");
    }
    else
    {
        CWriter.line("            .initialize = false, /*!< Initialize    */");
