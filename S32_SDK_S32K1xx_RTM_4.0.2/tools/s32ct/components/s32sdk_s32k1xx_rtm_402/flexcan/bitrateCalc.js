   function calcBautRate() 
   {
	...
	var fd = configSet.getChildById("flexcanCfg.flexcan_cfg_fd_enable");
	var fd_value = fd.getValue();
    var bit2Seg = configSet.getChildById("flexcanCfg.flexcan_bitrate2ts");
	var bit2Seg_value = bit2Seg.getValue();
    var peClk = configSet.getChildById("flexcanCfg.pe_clock_frequency");
	var peClk_value = peClk.getValue();
    if (bit2Seg_value == true)
    {
        var bitrate = configSet.getChildById("flexcanCfg.flexcan_cfg_time_segments2.flexcan_cfg_bitrate.input_nominal_bitrate");
        var bitrate_value = bitrate.getValue();
        var sample_point = configSet.getChildById("flexcanCfg.flexcan_cfg_time_segments2.flexcan_cfg_bitrate.input_nominal_sampling");
        var sample_point_value = sample_point.getValue();
    }
    else
    {
        var bitrate = configSet.getChildById("flexcanCfg.flexcan_cfg_time_segments1.flexcan_cfg_bitrate.bitrate");
        var bitrate_value = bitrate.getValue();
        var sample_point = configSet.getChildById("flexcanCfg.flexcan_cfg_time_segments1.flexcan_cfg_bitrate.flexcan_cfg_samplingPoint");
        var sample_point_value = sample_point.getValue();
    }
    ...
    {  
        ...
        if (iteration == 1)
        {
            ...
            pseg1_max = 7;
            pseg2_max = 7;
            bitrate = configSet.getChildById("flexcanCfg.flexcan_cfg_time_segments2.flexcan_cfg_bitrate_cbt.input_data_bitrate");
            bitrate_value = bitrate.getValue();
            sample_point = configSet.getChildById("flexcanCfg.flexcan_cfg_time_segments2.flexcan_cfg_bitrate_cbt.input_data_sampling");
        ...
        else
        {
            ...
            pseg1_max = 7;
            pseg2_max = 7;
            bitrate = configSet.getChildById("flexcanCfg.flexcan_cfg_time_segments2.flexcan_cfg_bitrate.input_nominal_bitrate");
            bitrate_value = bitrate.getValue();
            sample_point = configSet.getChildById("flexcanCfg.flexcan_cfg_time_segments2.flexcan_cfg_bitrate.input_nominal_sampling");
            sample_point_value = sample_point.getValue();
        }
        ...
        if (found_values)
        {
            result.add(presdiv|0);
            result.add(pseg1|0);
            result.add(pseg2|0);
            result.add(propseg|0);
    ...
    result.add(peClk_value/4000.0 > configSet.getChildById("flexcanCfg.flexcan_cfg_time_segments2.flexcan_cfg_bitrate_cbt.input_data_bitrate").getValue());
	return result;
   }
