...
function addHeaderCommentsFreeRTOSConfig(){
    var outputString = "";
        outputString += "/* ###################################################################\n";
        outputString += "**     Copyright 2019 NXP \n";
...
function addFooterCommentsFreeRTOSConfig(){
    ...
    outputString += "#endif\n";
    outputString += "\n";
    outputString += "/* Cortex-R52 specific definitions. */\n";
    outputString += "#ifdef __NVIC_PRIO_BITS\n";
    outputString += "  #define configPRIO_BITS                         __NVIC_PRIO_BITS\n";
    outputString += "#else\n";
    outputString += "  #define configPRIO_BITS                         5\n";
    outputString += "#endif\n";
    outputString += "\n";
    outputString += "#ifndef configMAX_SYSCALL_INTERRUPT_PRIORITY\n";
    outputString += "  #define configMAX_SYSCALL_INTERRUPT_PRIORITY    ((configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY << (8 - configPRIO_BITS)) & 0xf8)\n";
    outputString += "#endif\n";
    outputString += "\n";
...
function freertos_CodeGenerator(uiConfig) {
    ...
    this.genHCfgFile = function(hCfgFileSections) {
        ...
        // if(featureDefined('FEATURE_FREERTOS_USE_PIT')==true)&&(getFeature('FEATURE_FREERTOS_USE_PIT')==1){
            var configUSE_PIT_CHANNEL = initStructGeneral.getChildById("configUSE_PIT_CHANNEL").getValue();
        // }
        ...
        // if(featureDefined('FEATURE_FREERTOS_USE_PIT')==true)&&(getFeature('FEATURE_FREERTOS_USE_PIT')==1){
            outputString += addDefineFreeRTOSConfig("configUSE_PIT_CHANNEL", "( " + configUSE_PIT_CHANNEL + "U )", 51);
        // }
