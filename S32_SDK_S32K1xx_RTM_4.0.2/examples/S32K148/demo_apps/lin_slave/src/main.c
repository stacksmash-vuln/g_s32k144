...
#define LPUART_PORT_NAME          (PORTA)
#define LPUART_Rx_INDEX           (2u)
#define LPUART_PORT_IRQn          (PORTA_IRQn)
...
lpspi_master_config_t lpspi_MasterConfig0 =
{
    .bitsPerSec       = 500000U,
    .whichPcs         = LPSPI_PCS0,
...
void SBCInit(void)
{
    LPSPI_DRV_MasterInit(1,&lpspiState,&lpspi_MasterConfig0);
    /* Initialize SBC */
    UJA113X_RegisterDevice(1, &drvConfig);
    UJA113X_SetMode(1, MC_NORMAL);
    UJA113X_SetLin(1, &lincConfig);
}
#endif
...
void RXPIN_IRQHandler(void)
{
	...
	PINS_DRV_ClearPortIntFlagCmd(LPUART_PORT_NAME);
}
...
int main(void)
{
    ...
    INT_SYS_InstallHandler(LPUART_PORT_IRQn, RXPIN_IRQHandler, (isr_t *)NULL);
    /* Enable PORT interrupt */
    INT_SYS_EnableIRQ(LPUART_PORT_IRQn);
