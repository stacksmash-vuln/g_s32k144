...
void ADC_PWM_Section(void)
{
    ...
    while(1)
    {
        ...
        if(newData)
        {
            FTM_DRV_UpdatePwmChannel(INST_FLEXTIMER_PWM1, 1U, FTM_PWM_UPDATE_IN_DUTY_CYCLE, (uint16_t)SINE_TO_DUTYCYCLE(RGB[R]),0U, false);
            FTM_DRV_UpdatePwmChannel(INST_FLEXTIMER_PWM1, 2U, FTM_PWM_UPDATE_IN_DUTY_CYCLE, (uint16_t)SINE_TO_DUTYCYCLE(RGB[G]),0U, false);
            FTM_DRV_UpdatePwmChannel(INST_FLEXTIMER_PWM1, 3U, FTM_PWM_UPDATE_IN_DUTY_CYCLE, (uint16_t)SINE_TO_DUTYCYCLE(RGB[B]),0U, true);
            newData = false;
        }
