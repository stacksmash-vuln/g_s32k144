...
/* ###################################################################
**     Filename    : main.c
**     Project     : lin_slave_baremetal_s32k148
...
void SBCInit(void)
{
    ...
    /* Initialize SBC */
    UJA113X_RegisterDevice(INST_LPSPI_1, &drvConfig);
    UJA113X_SetMode(INST_LPSPI_1, MC_NORMAL);
    UJA113X_SetLin(INST_LPSPI_1, &lincConfig);
}
#endif
...
void RXPIN_IRQHandler(void)
{
    ...
    if(PINS_DRV_GetPortIntFlag(PORTA) & (1UL << 2))
    {
...
void lin_slave_baremetal_task(void)
{
    ...
    LIN_DRV_Init(INST_LIN_NUM, &lin0_InitConfig0, &lin0_State);
    ...
    for (; ; )
    {
        ...
        if((status == STATUS_SUCCESS) && (0U == byteRemain) && (FRAME_SLAVE_RECEIVE_DATA == lin0_State.currentId))
        {
...
int main(void)
{
  /* Write your local variable definition here */
     ...
     */
    PINS_DRV_SetPinIntSel(PORTA, 2UL, PORT_INT_EITHER_EDGE);
    /* Install the handler for the LPUART RX pin */
    INT_SYS_InstallHandler(PORTA_IRQn, RXPIN_IRQHandler, (isr_t *)NULL);
