...
/* ###################################################################
**     Filename    : main.c
**     Project     : lin_master_baremetal_s32k144
...
void BTNPORT_IRQHandler(void)
{
    ...
    if (PINS_DRV_GetPortIntFlag(BTN_PORT_NAME) & (1 << PORT_BTN0_INDEX))
    {
        ...
        if((counter % 2U) == 1U)
        {
            LIN_DRV_MasterSendHeader(INST_LIN2, FRAME_SLAVE_RECEIVE_DATA);
        }
        else
        {
            LIN_DRV_MasterSendHeader(INST_LIN2, FRAME_MASTER_RECEIVE_DATA);
        }
    ...
    if (PINS_DRV_GetPortIntFlag(BTN_PORT_NAME) & (1U << PORT_BTN1_INDEX))
    {
        ...
        if(LIN_NODE_STATE_SLEEP_MODE == LIN_DRV_GetCurrentNodeState(INST_LIN2))
        {
            /* Send wakeup signal */
            LIN_DRV_SendWakeupSignal(INST_LIN2);
        ...
        else
        {
            ...
            LIN_DRV_MasterSendHeader(INST_LIN2, FRAME_GO_TO_SLEEP);
    ...
    switch (lin1_State->currentEventId)
    {
        case LIN_PID_OK:
            ...
            if(FRAME_MASTER_RECEIVE_DATA == lin1_State->currentId)
            {
                ...
                LIN_DRV_ReceiveFrameData(INST_LIN2, rxBuff, sizeof(rxBuff));
            }
...
void lin_master_baremetal_task(void)
{
    ...
    for (;;)
    {
        status = LIN_DRV_GetReceiveStatus(INST_LIN2, &byteRemain);

        if((status == STATUS_SUCCESS) && (0U == byteRemain) && (FRAME_MASTER_RECEIVE_DATA == lin2_State.currentId))
        {
