...
/* ###################################################################
**     Filename    : main.c
**     Project     : lin_slave_baremetal_s32k146
...
void RXPIN_IRQHandler(void)
{
	...
	if(PINS_DRV_GetPortIntFlag(PORTD) & (1UL << 6))
    {
    ...
    switch (lin1_State->currentEventId)
    {
        case LIN_PID_OK:
            ...
            if(FRAME_MASTER_RECEIVE_DATA == lin1_State->currentId)
            {
                ...
                LIN_DRV_SendFrameData(INST_LIN2, txBuff2, sizeof(txBuff2));
            }
...
void lin_slave_baremetal_task(void)
{
    ...
    for (; ; )
    {
        ...
        if((status == STATUS_SUCCESS) && (0U == byteRemain) && (FRAME_SLAVE_RECEIVE_DATA == lin2_State.currentId))
        {
...
int main(void)
{
  /* Write your local variable definition here */
    ...
    INT_SYS_InstallHandler(PORTD_IRQn, RXPIN_IRQHandler, (isr_t *)NULL);
