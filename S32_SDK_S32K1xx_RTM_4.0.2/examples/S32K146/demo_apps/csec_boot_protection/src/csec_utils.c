...
status_t computeM1M2M3(uint8_t *authKey, csec_key_id_t authId, csec_key_id_t keyId, const uint8_t *key, uint32_t counter,
                                uint8_t *uid, uint8_t *m1, uint8_t *m2, uint8_t *m3, bool bootProtection)
{
    status_t stat;
    int i;
...
bool setAuthKey(void)
{
    ...
    stat = computeM1M2M3(g_emptyKey, g_authId, CSEC_MASTER_ECU, g_authIdKey, 1, uid, m1, m2, m3, 0);
    if (stat != STATUS_SUCCESS)
        return false;
...
bool loadKey(csec_key_id_t keyId, uint8_t *keyNew, uint8_t counter, bool bootProtection)
{
    ...
    stat = computeM1M2M3(g_authIdKey, g_authId, keyId, keyNew, counter, uid, m1, m2, m3, bootProtection);
    if (stat != STATUS_SUCCESS)
        return false;
