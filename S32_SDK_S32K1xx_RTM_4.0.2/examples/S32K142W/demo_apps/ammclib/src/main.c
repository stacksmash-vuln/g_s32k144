 ...
 * S32K14xCVD-Q064 with S32K-MB you need to remove C preprocessor EVB.
 */

#ifdef EVB
    #define INST_ADCONV           0U
    #define ADC_IRQn              ADC0_IRQn
    #define LPUART_RxTx_IRQn      LPUART1_RxTx_IRQn
    #define INST_LPUART           1U
#else
    #define INST_ADCONV           INST_ADCONV1
    #define ADC_IRQn              ADC1_IRQn
    #define LPUART_RxTx_IRQn      LPUART2_RxTx_IRQn
    #define INST_LPUART           INST_LPUART1
#endif
...
void ADC_ISR(void)
{
    ADC_DRV_GetChanResult(INST_ADCONV, 0U, (uint16_t *)&adcRawValue);
...
void ADC_PWM_Section(void)
{
    ...
    LPUART_DRV_SendDataBlocking(INST_LPUART, (uint8_t *)ADC_PWM_TXT, strlen(ADC_PWM_TXT), LPUART_TX_TIMEOUT);
    ...
    ADC_DRV_AutoCalibration(INST_ADCONV);
    ADC_DRV_ConfigConverter(INST_ADCONV, &adConv1_ConvConfig0);
#ifdef EVB
    adConv1_ChnConfig0.channel = ADC_INPUTCHAN_EXT9;
#endif
    ADC_DRV_ConfigChan(INST_ADCONV, 0U, &adConv1_ChnConfig0);
    /* Install the corresponding ISR and enable the interrupt */
    INT_SYS_InstallHandler(ADC_IRQn, &ADC_ISR, (isr_t *)NULL);
    INT_SYS_EnableIRQ(ADC_IRQn);

    /* Give control of communication to FreeMASTER */
    INT_SYS_InstallHandler(LPUART_RxTx_IRQn, &FMSTR_Isr, (isr_t *)NULL);
...
void Basic_Section(void)
{
    ...
    /* Send instructions to terminal */
    LPUART_DRV_SendDataBlocking(INST_LPUART, (uint8_t *)BASIC_TXT, strlen(BASIC_TXT), LPUART_TX_TIMEOUT);
    ...
    while(true)
    {
        ...
        while(true)
        {
            LPUART_DRV_ReceiveDataBlocking(INST_LPUART, (uint8_t *)buffer, 1024, LPUART_RX_TIMEOUT);
            if (strlen(buffer) != 0)
            {
                break;
            }
        ...
        if (status == 1)
        {
            ...
            LPUART_DRV_SendDataBlocking(INST_LPUART, (uint8_t *)out, strlen(out), LPUART_TX_TIMEOUT);
...
int main(void)
{
     ...
     *  -   See clock_manager component for details
     */
     ...
     */
    LPUART_DRV_Init(INST_LPUART, &lpuart1_State, &lpuart1_InitConfig0);
    ...
    {
        /* Print menu */
        LPUART_DRV_SendDataBlocking(INST_LPUART, (uint8_t *)WELCOME_TXT, strlen(WELCOME_TXT), LPUART_TX_TIMEOUT);
        ...
        if(strlen(buffer) > 2)
        {
            ...
            LPUART_DRV_SendDataBlocking(INST_LPUART, (uint8_t *)CLR_SCR, strlen(CLR_SCR), LPUART_TX_TIMEOUT);
            LPUART_DRV_SendDataBlocking(INST_LPUART, (uint8_t *)ERROR_TXT, strlen(ERROR_TXT), LPUART_TX_TIMEOUT);
        }
        else
        {
            switch(buffer[0])
