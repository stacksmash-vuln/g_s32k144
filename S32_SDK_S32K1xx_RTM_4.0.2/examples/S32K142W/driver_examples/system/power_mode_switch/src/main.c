...
/* ###################################################################
**     Filename    : main.c
**     Project     : power_mode_switch_s32k142w
**     Processor   : S32K142W-64
**     Abstract    :
**         Main module.
...
#define VLPS  (4u) /* Very low power stop */
...
void ButtonISR(void)
{
    ...
    /* Board EVB - greed led on */
    PINS_DRV_WritePin(LED_PORT, GREEN_LED_PIN, 1);
    /* Board EVB - red led off */
    PINS_DRV_WritePin(LED_PORT, RED_LED_PIN, 0);
}
...
int main(void)
{
    ...
    INT_SYS_InstallHandler(PORTD_IRQn, ButtonISR, (isr_t *)0);

    /* Enable button port IRQ */
    INT_SYS_EnableIRQ(PORTD_IRQn);

    /*  EVB board - green led on */
    PINS_DRV_WritePin(LED_PORT, GREEN_LED_PIN, 1);

    /*  EVB board - red led off */
    PINS_DRV_WritePin(LED_PORT, RED_LED_PIN, 0);
    ...
    while(1)
    {
        ...
        case '3':
            ...
            /* EVB green led off */
            PINS_DRV_WritePin(LED_PORT, GREEN_LED_PIN, 0);

            /* EVB red led on */
            PINS_DRV_WritePin(LED_PORT, RED_LED_PIN, 1);
