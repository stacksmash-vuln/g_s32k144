...
int main(void)
{
    ...
    /* Example Description
     * Potentiometer voltage is read using ADC0- channel 9. The value is send to
     * LPSPI1 from LPSPI0. The read value represent duty for PWM (FTM0-channel 7).
     * SPI wiring:
     * B5-D3
     * D15-D0
     * B4-D1
    ...
    LPSPI_DRV_MasterInit(INST_LPSPI_1, &lpspi_1State, &lpspiMasterConfig0);
    /* Initialize LPSPI1 (Receive)*/
    LPSPI_DRV_SlaveInit(INST_LPSPI_2, &lpspi_2State, &lpspiSlaveConfig1);
    /* Initialize ADC */
    ...
    while(1)
    {
        ...
        FTM_DRV_UpdatePwmChannel(INST_FLEXTIMER_PWM_1, 7U, FTM_PWM_UPDATE_IN_DUTY_CYCLE, masterDataReceive<<8, 0U, true);
    }
