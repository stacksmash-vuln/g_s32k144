...
/* ###################################################################
**     Filename    : main.c
**     Project     : lin_master_baremetal_s32k142w
**     Processor   : S32K142W
**     Version     : Driver 01.00
**     Compiler    : GNU C Compiler
**     Date/Time   : 2020-02-24, 10:58, # CodeGen: 0
**     Abstract    :
**         Main module.
...
void BTNPORT_IRQHandler(void)
{
    ...
    if (PINS_DRV_GetPortIntFlag(BTN_PORT_NAME) & (1U << PORT_BTN1_INDEX))
    {
        ...
        {
            /* Send wakeup signal */
            LIN_DRV_SendWakeupSignal(INST_LIN);
        ...
        else
        {
            ...
            LIN_DRV_MasterSendHeader(INST_LIN, FRAME_GO_TO_SLEEP);
    ...
    switch (lin_State->currentEventId)
    {
        case LIN_PID_OK:
            ...
            if(FRAME_SLAVE_RECEIVE_DATA == lin_State->currentId)
            {
                ...
                LIN_DRV_SendFrameData(INST_LIN, txBuff1, sizeof(txBuff1));
            }
...
void lin_master_baremetal_task(void)
{
    ...
    for (;;)
    {
        ...
        if((status == STATUS_SUCCESS) && (0U == byteRemain) && (FRAME_MASTER_RECEIVE_DATA == lin_State.currentId))
        {
