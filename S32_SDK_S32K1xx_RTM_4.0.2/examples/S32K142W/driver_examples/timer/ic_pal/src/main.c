...
#ifdef EVB
    #define INST_LPUART     1
#else
    #define INST_LPUART     2
#endif
...
void print(const char * sourceStr)
{
    ...
    LPUART_DRV_SendData(INST_LPUART, (uint8_t *)sourceStr, strlen(sourceStr));
    while (LPUART_DRV_GetTransmitStatus(INST_LPUART, &bytesRemaining) != STATUS_SUCCESS);
}
...
int main(void)
{
    ...
    LPUART_DRV_Init(INST_LPUART, &lpUartState0, &lpUartInitConfig0);
    ...
    /* Setup input capture for FTM 1 channel 0 */
    IC_Init(&ic_pal_1_instanceConfig, &ic_pal_1_InitConfig);

    /* Start PWM channel on FTM 0 channel 0 */
    ...
    for ( ;; )
    {
        ...
        /* Wait for user input */
        LPUART_DRV_ReceiveDataBlocking(INST_LPUART, (uint8_t *)&txBuff, 1U, 1 << 31U);
