...
void print(const char * sourceStr)
{
    ...
    while (LPUART_DRV_GetTransmitStatus(INST_LPUART_0, &bytesRemaining) != STATUS_SUCCESS);
}
...
int main(void)
{
    ...
    LPUART_DRV_Init(INST_LPUART_0, &lpUartState0, &lpUartInitConfig0);
    ...
    for ( ;; )
    {
        ...
        /* Wait for user input */
        LPUART_DRV_ReceiveDataBlocking(INST_LPUART_0, (uint8_t *)&txBuff, 1U, 1 << 31U);
