...
#define BASIC_TXT       "\nBasic mathematical functions selected.\n"    \
                        ...
                        " ! ! ! Accepted range is [-1, 1)  ! ! !\n"     \
                        "\nTo go back to the menu type exit.\n"         \
                        "\n"

#define PWM_TXT         "\nPWM generation selected.\n"                                  \
...
float Generate_Sine_Signal(float frequency, float amplitude, uint32_t sample, float phase)
{
     ...
     */
    sineArgInDegrees = SINE_PARAM(frequency / 1000, sample_time) * sample + phase;
    /* Convert the angle to radians */
    normalizedAngle = Normalize_Angle(sineArgInDegrees);
    /* Calculate the sample sine value */
    return amplitude * (GFLIB_Sin(FRAC16(normalizedAngle/PI), GFLIB_SIN_DEFAULT) / 32768.0);
}
 ...
 */
void PWM_Section(void)
{
    /* Send instructions to terminal */
    LPUART_DRV_SendDataBlocking(INST_LPUART1, (uint8_t *) PWM_TXT, strlen(PWM_TXT),
    LPUART_TX_TIMEOUT);
    ...
    INT_SYS_InstallHandler(LPUART0_RxTx_IRQn, &FMSTR_Isr, (isr_t *) NULL);
    ...
    while (1)
    {
        ...
        if (newData)
        {
            FTM_DRV_UpdatePwmChannel(INST_FLEXTIMER_PWM1, 0U, FTM_PWM_UPDATE_IN_DUTY_CYCLE,
                    (uint16_t) SINE_TO_DUTYCYCLE(RGB[R]), 0U, false);
            FTM_DRV_UpdatePwmChannel(INST_FLEXTIMER_PWM1, 1U, FTM_PWM_UPDATE_IN_DUTY_CYCLE,
                    (uint16_t) SINE_TO_DUTYCYCLE(RGB[G]), 0U, false);
            FTM_DRV_UpdatePwmChannel(INST_FLEXTIMER_PWM1, 6U, FTM_PWM_UPDATE_IN_DUTY_CYCLE,
                    (uint16_t) SINE_TO_DUTYCYCLE(RGB[B]), 0U, true);
            newData = false;
        }
...
float Evaluate_Expression(char *str, tFloat *value)
{
    ...
    *value = 0.0;
    ...
    for (tok = strtok(str, " "); tok; tok = strtok(NULL, " "))
    {
        ...
        {
            tok = strtok(NULL, " ");
            *value = MLIB_Add(FRAC16(*value), FRAC16(atof(tok))) / 32768.0;
        }
        else if (strcmp(tok, "-") == 0)
        {
            ...
            if (*value == 0)
            {
                *value = MLIB_Neg(FRAC16(atof(tok))) / 32768.0;
            }
            else
            {
                *value = MLIB_Sub(FRAC16(*value), FRAC16(atof(tok))) / 32768.0;
            }
        }
        else if ((strcmp(tok, "*") == 0))
        {
            tok = strtok(NULL, " ");
            *value = MLIB_Mul(FRAC16(*value), FRAC16(atof(tok))) / 32768.0;
        }
        else if ((strcmp(tok, "/") == 0))
        {
            tok = strtok(NULL, " ");
            *value = MLIB_Div(FRAC16(*value), FRAC16(atof(tok))) / 32768.0;
        }
        else if ((strcmp(tok, "sin") == 0))
        {
            tok = strtok(NULL, " ");
            *value = GFLIB_Sin(FRAC16(atof(tok) / 180.0f), GFLIB_SIN_DEFAULT) / 32768.0;
        }
        else if ((strcmp(tok, "cos") == 0))
        {
            tok = strtok(NULL, " ");
            *value = GFLIB_Cos(FRAC16(atof(tok) / 180.0f), GFLIB_COS_DEFAULT) / 32768.0;
        }
        else if ((strcmp(tok, "tan") == 0))
        {
            tok = strtok(NULL, " ");
            *value = GFLIB_Tan(FRAC16(atof(tok) / 180.0f), GFLIB_TAN_DEFAULT) / 32768.0;
        }
        else if ((strcmp(tok, "asin") == 0))
        {
            tok = strtok(NULL, " ");
            *value = ((GFLIB_Asin(FRAC16(atof(tok)), GFLIB_ASIN_DEFAULT) / 32768.0) * 180.0f);
        }
        else if ((strcmp(tok, "acos") == 0))
        {
            tok = strtok(NULL, " ");
            *value = ((GFLIB_Acos(FRAC16(atof(tok)) , GFLIB_ACOS_DEFAULT) / 32768.0) * 180.0f);
        }
        else if ((strcmp(tok, "atan") == 0))
        {
            tok = strtok(NULL, " ");
            *value = ((GFLIB_Atan(FRAC16(atof(tok)), GFLIB_ATAN_DEFAULT) / 32768.0) * 180.0f);
        }
        else if ((strcmp(tok, "sqrt") == 0))
        {
            tok = strtok(NULL, " ");
            *value = GFLIB_Sqrt(FRAC16(atof(tok))) / 32768.0;
        }
        else if ((strcmp(tok, "abs") == 0))
        {
            tok = strtok(NULL, " ");
            *value = MLIB_Abs(FRAC16(atof(tok))) / 32768.0;
        }
...
void Basic_Section(void)
{
    ...
    /* Send instructions to terminal */
    LPUART_DRV_SendDataBlocking(INST_LPUART1, (uint8_t *) BASIC_TXT, strlen(BASIC_TXT),
    LPUART_TX_TIMEOUT);
    ...
    while (true)
    {
        ...
        while (true)
        {
            LPUART_DRV_ReceiveDataBlocking(INST_LPUART1, (uint8_t *) buffer, 1024,
            LPUART_RX_TIMEOUT);
            if (strlen(buffer) != 0)
            {
                break;
        ...
        if (status == 1)
        {
            ...
            LPUART_DRV_SendDataBlocking(INST_LPUART1, (uint8_t *) out, strlen(out),
            LPUART_TX_TIMEOUT);
...
int main(void)
{
  ...
  {
    /* Print menu */
    LPUART_DRV_SendDataBlocking(INST_LPUART1, (uint8_t *) WELCOME_TXT, strlen(WELCOME_TXT),
    LPUART_TX_TIMEOUT);
    ...
    if (strlen(buffer) > 2)
    {
      ...
      LPUART_DRV_SendDataBlocking(INST_LPUART1, (uint8_t *) CLR_SCR, strlen(CLR_SCR),
      LPUART_TX_TIMEOUT);
      LPUART_DRV_SendDataBlocking(INST_LPUART1, (uint8_t *) ERROR_TXT, strlen(ERROR_TXT),
      LPUART_TX_TIMEOUT);
    }
    else
    {
      switch (buffer[0])
      {
        ...
        default: /* Unavailable */
          LPUART_DRV_SendDataBlocking(INST_LPUART1, (uint8_t *) CLR_SCR, strlen(CLR_SCR),
          LPUART_TX_TIMEOUT);
          LPUART_DRV_SendDataBlocking(INST_LPUART1, (uint8_t *) ERROR_TXT,
                  strlen(ERROR_TXT), LPUART_TX_TIMEOUT);
          break;
      }
    }
    /* clear input buffer */
