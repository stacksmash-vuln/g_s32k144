...
#define EVB
    ...
    #define ADC_CHN         9U
	#define LPUART_INSTANCE 1UL
#else
    #define ADC_INSTANCE    1
    #define ADC_CHN         10U
	#define LPUART_INSTANCE 2UL
#endif
...

{
    ...
    ADC_DRV_GetChanResult(ADC_INSTANCE, ADC_CHANNEL, (uint16_t *)&loc_conversionResult);
...
uint16_t getVrefValue(void)
{
     ...
     * interrupt.
     */
    ADC_DRV_ConfigChan(ADC_INSTANCE, ADC_CHANNEL, &adConv1_ChnConfig1);

    /* Wait for the ADC to complete the conversion */
    ADC_DRV_WaitConvDone(ADC_INSTANCE);

    /* Get the conversion result */
    ADC_DRV_GetChanResult(ADC_INSTANCE, ADC_CHANNEL, (uint16_t *)&Vbg_measured_with_VrefAlt);
     ...
     * interrupt.
     */
    ADC_DRV_ConfigChan(ADC_INSTANCE, ADC_CHANNEL, &adConv1_ChnConfig2);

    /* Wait for the ADC to complete the conversion */
    ADC_DRV_WaitConvDone(ADC_INSTANCE);

    /* Get the conversion result */
    ADC_DRV_GetChanResult(ADC_INSTANCE, ADC_CHANNEL, (uint16_t *)&VrefAltH_measured_with_Vbg);
     ...
     * a resolution of 12 bits
     */
    ADC_DRV_ConfigConverter(ADC_INSTANCE, &adConv1_ConvConfig3);

    /* Configure the ADC channel and disable the conversion complete
     * interrupt.
     */
    ADC_DRV_ConfigChan(ADC_INSTANCE, ADC_CHANNEL, &adConv1_ChnConfig3);

    /* Wait for the ADC to complete the conversion */
    ADC_DRV_WaitConvDone(ADC_INSTANCE);

    /* Get the conversion result */
    ADC_DRV_GetChanResult(ADC_INSTANCE, ADC_CHANNEL, (uint16_t *)&VrefAltL_measured_with_Vbg);
...
void InitPeripherals(void)
{
     ...
     */
    ADC_DRV_ConfigHwCompare(ADC_INSTANCE, &adConv1_HwCompConfig0);

    /* Configure the ADC channel and enable the conversion complete
     * interrupt.
     */
    ADC_DRV_ConfigChan(ADC_INSTANCE, ADC_CHANNEL, &adConv1_ChnConfig0);

    /* Enable ADC interrupt and install an ISR */
    IRQn_Type adc_irq_id = ADC_DRV_GetInterruptNumber(ADC_INSTANCE);
    ...
    LPUART_DRV_Init(LPUART_INSTANCE, &lpuart1_State, &lpuart1_InitConfig0);
}
...
int main(void)
{
	/* Local variable used to store the bar graph */
	    ...
	    /* Setup system drivers */
	        ...
	        /* Welcome screen */
	        LPUART_DRV_SendDataBlocking(LPUART_INSTANCE, (uint8_t *) WELCOME_TXT, (uint32_t)sizeof(WELCOME_TXT), LPUART_MAX_TIMEOUT);
	        ...
	        LPUART_DRV_SendDataBlocking((uint32_t)LPUART_INSTANCE, (const uint8_t *)buff, strlen((char *)buff), LPUART_MAX_TIMEOUT);

	        /* Acknowledge Welcome screen */
	        LPUART_DRV_SendDataBlocking(LPUART_INSTANCE, (uint8_t *) ACK_WELCOME, (uint32_t)sizeof(ACK_WELCOME), LPUART_MAX_TIMEOUT);
	        /* wait ack keys */
	        while (1)
	        {
	            /* get UART data, blocking */
	            if (STATUS_SUCCESS == LPUART_DRV_ReceiveDataBlocking(LPUART_INSTANCE, &rxbuff[0], 1, LPUART_MAX_TIMEOUT*5))
	            {
	        ...
	        /* Acknowledge Welcome screen */
	        LPUART_DRV_SendDataBlocking(LPUART_INSTANCE, (uint8_t *) ACK_OK_WELCOME, (uint32_t)sizeof(ACK_OK_WELCOME), LPUART_MAX_TIMEOUT);

	        /* Usage, yet again */
	        LPUART_DRV_SendDataBlocking(LPUART_INSTANCE, (uint8_t *) EXAMPLE_USAGE, (uint32_t)sizeof(EXAMPLE_USAGE), LPUART_MAX_TIMEOUT);
	        ...
	        while (1)
	        {
	            ...
	            if (g_adcEvent == true)
	            {
	                ...
	                sprintf((char *)buff, ADC_BAR_TEMPLATE, ADC_INSTANCE, adConv1_ChnConfig0.channel, barbuff, g_vin, g_conversionResult);
	                /* Send the data to the host PC, no stalling */
	                LPUART_DRV_SendDataBlocking((uint32_t)LPUART_INSTANCE, (const uint8_t *)buff, strlen((char *)buff), LPUART_MAX_TIMEOUT);
