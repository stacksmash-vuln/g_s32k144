...
/* ###################################################################
**     Filename    : main.c
**     Project     : mpu_pal_memory_protection_s32k144w
**     Processor   : s32k144w
**     Abstract    :
**         Main module.
...
#define EVB
    ...
    #define LED0            0u         /* pin PTE0 - LED GREEN on DEV-KIT */
    #define LED1            7U         /* pin PTE7 - LED RED on DEV-KIT */
    #define SW_GPIO         PTD        /* SW GPIO type */
    #define SW              02U         /* pin PTD2 - SW2_BTN0 on DEV-KIT */
#else
    #define ON              0U         /* LED ON */
    ...
    #define SW_GPIO         PTB        /* SW GPIO type */
    #define SW              12U        /* pin PTB12 - SW7_BTN0 on Motherboard */
#endif


/* Protected address in Flash memory */
#define ADDRESS_PROTECT     0x0007FF04U
...
void HardFault_Handler(void)
{
    ...
    (void)MPU_EnableRegion(&mpu_pal1_Instance, 3U, true);
}
...
int main(void)
{
    ...
    *(uint32_t *)0xE000EDFC = 0x1000000;
    ...
    /* LEDs off */
    PINS_DRV_WritePin(LED_GPIO, LED0, OFF);
    PINS_DRV_WritePin(LED_GPIO, LED1, OFF);

    /* Check the initial state of the button */
    switchStatus = PINS_DRV_ReadPins(SW_GPIO) & (1U << SW);

    /* Initializes MPU module */
    returnCode = MPU_Init(&mpu_pal1_Instance, MPU_PAL1_REGION_CFG_CNT0, mpu_pal1_RegionConfig0);

    /* Check initialization */
    if (returnCode == STATUS_SUCCESS)
    {
        /* Turn on LED0 */
        PINS_DRV_WritePin(LED_GPIO, LED0, ON);
    }
    ...
    for(;;)
    {
        ...
        if (switchStatus != (PINS_DRV_ReadPins(SW_GPIO) & (1U << SW)))
        {
            ...
            (void)MPU_EnableRegion(&mpu_pal1_Instance, 3U, false);
        }
        ...
        status = MPU_GetError(&mpu_pal1_Instance, FEATURE_MPU_SLAVE_FLASH_BOOTROM, &reportedError);
        ...
        if (status)
        {
            ...
            if (status)
            {
                ...
                PINS_DRV_WritePin(LED_GPIO, LED1, ON);
                PINS_DRV_WritePin(LED_GPIO, LED0, OFF);
                break;
            }
        }
