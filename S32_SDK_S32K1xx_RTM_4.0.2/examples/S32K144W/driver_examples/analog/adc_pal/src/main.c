...
#define welcomeStr "\r\nThis is an example for ADC PAL: it will print the average value of the conversion results in groups of conversions."\
                   "\r\nMeasurements are done on ADC1 Input 10 (or ADC0 Input 9 for EVB)\r\n"
...
int main(void)
{
    ...
    /* Initialize and configure clocks
         ...
         */
        status = LPUART_DRV_Init(LPUART_INSTANCE, &lpUartState0, &lpUartInitConfig0);
        ...
        status = LPUART_DRV_Deinit(LPUART_INSTANCE);
        DEV_ASSERT(status == STATUS_SUCCESS);
}
