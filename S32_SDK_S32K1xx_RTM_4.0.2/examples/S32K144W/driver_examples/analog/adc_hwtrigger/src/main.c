...
#define welcomeStr "This is an ADC example, it will show you the value converted"\
                   "\r\nfrom ADC1 Input 10 (or ADC0 Input 9 for EVB)\r\n"
...
void print(const char *sourceStr)
{
    ...
    LPUART_DRV_SendData(LPUART_INSTANCE, (uint8_t *) sourceStr, strlen(sourceStr));
    /* Wait for transmission to be successful */
    while (LPUART_DRV_GetTransmitStatus(LPUART_INSTANCE, &bytesRemaining)
            != STATUS_SUCCESS)
    {
    }
}
...
int main(void)
{
    ...
    if (ADC_1_ConvConfig0.resolution == ADC_RESOLUTION_8BIT)
        adcMax = (uint16_t) (1 << 8);
    else if (ADC_1_ConvConfig0.resolution == ADC_RESOLUTION_10BIT)
     ...
     */
    if (LPUART_DRV_Init(LPUART_INSTANCE, &lpUartState2, &lpUartInitConfig2)
            != STATUS_SUCCESS)
        __asm("bkpt #255");
    ...
    DEV_ASSERT(ADC_1_ChnConfig0.channel == ADC_CHN);

    ADC_DRV_ConfigConverter(ADC_INSTANCE, &ADC_1_ConvConfig0);
    ADC_DRV_AutoCalibration(ADC_INSTANCE);
    ADC_DRV_ConfigChan(ADC_INSTANCE, 0UL, &ADC_1_ChnConfig0);
