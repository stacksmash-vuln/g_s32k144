...
/* ###################################################################
**     Filename    : main.c
**     Project     : lin_slave_baremetal_s32k144w
**     Processor   : S32K144w_64
...
#define EVB
...
#ifdef EVB
    #define LIN_RX_IRQn               (PORTD_IRQn)
    #define LIN_RX_PORT               (PORTD)
    #define LIN_RX_PIN                lin2_rx_PIN
    #define ON                        1U         /* LED ON */
    #define OFF                       0U         /* LED OFF */
    #define lin_InitConfig0           lin2_InitConfig0
    #define INST_LIN                  INST_LIN2
    #define lin_State                 lin2_State
    #define PORT_LED0_INDEX           (3u)
    #define PORT_LED1_INDEX           (7u)
    #define PORT_LED2_INDEX           (0u)
    ...
    #define PORT_BTN0_INDEX           (2u)
    #define PORT_BTN1_INDEX           (3u)
...
#else
    #define LIN_RX_IRQn               (PORTC_IRQn)
    #define LIN_RX_PORT               (PORTC)
    #define LIN_RX_PIN                lin1_rx_PIN
    #define ON                        0U         /* LED ON */
    #define OFF                       1U         /* LED OFF */
    #define lin_InitConfig0           lin1_InitConfig0
    #define INST_LIN                  INST_LIN1
    #define lin_State                 lin1_State
    ...
    #define PORT_BTN0_MASK            (0x1u << PORT_BTN0_INDEX)
    #define PORT_BTN1_MASK            (0x1u << PORT_BTN1_INDEX)
    #define BTN_GPIO_PORT             (PTB)
    #define BTN_PORT_NAME             (PORTB)
    #define BTN_PORT_IRQn             (PORTB_IRQn)
    #define USE_LIN_XCVR              (1)
    #define LIN_XCVR_ENABLE_PIN       (8UL)
    #define LIN_XCVR_ENABLE_MASK      (0x1u << LIN_XCVR_ENABLE_PIN)
    #define LIN_XCVR_ENABLE_GPIO_PORT (PTE)
#endif
 ...
 * @return dummy value
 */
uint32_t linTimerGetTimeIntervalCallback0(uint32_t *ns)
{
...
void RXPIN_IRQHandler(void)
{
    ...
    if(PINS_DRV_GetPortIntFlag(LIN_RX_PORT) & (1UL << LIN_RX_PIN))
    {
        ...
        {
            if (LIN_DRV_AutoBaudCapture(INST_LIN) == STATUS_SUCCESS)
            {
    ...
    PINS_DRV_ClearPortIntFlagCmd(LIN_RX_PORT);
}
...
*/
lin_callback_t CallbackHandler(uint32_t instance, lin_state_t * lin_State)
{
    ...
    switch (lin_State->currentEventId)
    {
        case LIN_PID_OK:
            ...
            if(FRAME_SLAVE_RECEIVE_DATA == lin_State->currentId)
            {
            ...
            if(FRAME_MASTER_RECEIVE_DATA == lin_State->currentId)
            {
                ...
                LIN_DRV_SendFrameData(INST_LIN, txBuff2, sizeof(txBuff2));
            }

            /* If PID is 0x03, salve node will go to sleep mode */
            if(FRAME_GO_TO_SLEEP == lin_State->currentId)
            {
                /* Go to sleep mode */
...
void lin_slave_baremetal_task(void)
{
    ...
    LIN_DRV_Init(INST_LIN, &lin_InitConfig0, &lin_State);
    ...
    for (; ; )
    {
        status = LIN_DRV_GetReceiveStatus(INST_LIN, &byteRemain);

        if((status == STATUS_SUCCESS) && (0U == byteRemain) && (FRAME_SLAVE_RECEIVE_DATA == lin_State.currentId))
        {
            ...
            {
                /* Turn off Green LED */
                PINS_DRV_WritePin(LED_GPIO_PORT, PORT_LED2_INDEX, OFF);
                /* Turn on Red LED */
                PINS_DRV_WritePin(LED_GPIO_PORT, PORT_LED1_INDEX, ON);
                /* Turn off Blue LED */
                PINS_DRV_WritePin(LED_GPIO_PORT, PORT_LED0_INDEX, OFF);
            }
            else
            {
                /* Turn on Green LED */
                PINS_DRV_WritePin(LED_GPIO_PORT, PORT_LED2_INDEX, ON);
                /* Turn off Red LED */
                PINS_DRV_WritePin(LED_GPIO_PORT, PORT_LED1_INDEX, OFF);
                /* Turn off Blue LED */
                PINS_DRV_WritePin(LED_GPIO_PORT, PORT_LED0_INDEX, OFF);
            }
        }
        ...
        if (true == wakeupSignalFlag)
        {
            ...
            PINS_DRV_WritePin(LED_GPIO_PORT, PORT_LED1_INDEX, OFF);
            /* Turn on Blue LED */
            PINS_DRV_WritePin(LED_GPIO_PORT, PORT_LED0_INDEX, ON);
        }

        /* Check node state */
        if (LIN_NODE_STATE_SLEEP_MODE == LIN_DRV_GetCurrentNodeState(INST_LIN))
        {
            /* Turn off all LEDs */
            PINS_DRV_WritePin(LED_GPIO_PORT, PORT_LED1_INDEX, OFF);
            PINS_DRV_WritePin(LED_GPIO_PORT, PORT_LED0_INDEX, OFF);
...
int main(void)
{
  /* Write your local variable definition here */
     ...
     */
    PINS_DRV_SetPinIntSel(LIN_RX_PORT, LIN_RX_PIN, PORT_INT_EITHER_EDGE);
    /* Install the handler for the LPUART RX pin */
    INT_SYS_InstallHandler(LIN_RX_IRQn, RXPIN_IRQHandler, (isr_t *)NULL);
    /* Enable PORT interrupt */
    INT_SYS_EnableIRQ(LIN_RX_IRQn);

    /* Initialize LPTMR */
    LPTMR_DRV_Init(INST_LPTMR_0, &LPTMR_0_config0, false);
